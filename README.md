...
注意点:
PhantomX を 68030/68040 モードで使用する場合で、060turbo.sys 0.58 を使ってハイメモリRAMDISKを確保する場合、`-ld`のRAMDISKオプション以外は指定しないようにしてください。大きいファイルをロードに失敗する可能性があります。
...
プレイリストはカーソルの左右またはジョイパッドの左右で切り替えが可能です。

カーソルバーはカーソルキーの上下またはジョイパッドの上下で動かせます。その他ページ送りやリスト先頭・末尾へのジャンプなどもありますのでヘルプメッセージを参照してください。
なお、ヘルプに書かれていませんが、emacsキーバインドおよびviキーバインドの一部も対応しています。

リターンキーを押すかジョイパッドAボタンで選択されているMACSデータを再生します。ロード中に`SHIFT`キーを押しつづけるとキャンセル可能です。

---

`r`キーまたはジョイパッドBボタンを押すとリピートモードを切り替えることができます。(詳細後述)
...
`ESC`キー、`q`キー、F10キー、ジョイパッド上+AB同時押しのいずれかで終了します。
...
- X68000XVI 実機 + PhantomX 1.03c (68030モード・ハイメモリ768MB・VDISK・4B) + Mercury Unit V3 + PCM8PP.X
...
* 0.4.4 (2023/08/12) ... PhantomX 1.03c で再生が停止してしまう問題に対処した
* 0.4.3 (2023/07/31) ... 微修正
